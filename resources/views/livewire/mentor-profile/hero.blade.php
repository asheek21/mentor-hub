 <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8 mb-8">
     <div class="flex flex-col md:flex-row items-start md:items-center space-y-4 md:space-y-0 md:space-x-6">
         <img class="h-24 w-24 rounded-full" src="{{ $mentor->profile_picture }}"
             alt="{{ $mentor->full_name }}">
         <div class="flex-1">
             <div class="flex items-center space-x-3 mb-2">
                 <h1 class="text-3xl font-bold text-gray-900">{{ $mentor->full_name }}</h1>
                 <span class="bg-green-100 text-green-800 text-sm px-3 py-1 rounded-full">Available</span>
             </div>
             <p class="text-xl text-gray-600 mb-3">{{ $mentor->mentorProfile->current_role }}</p>
             <div class="flex items-center space-x-4 mb-4">
                 <div class="flex items-center">
                     ...
                     <span class="text-lg text-gray-600 ml-2">{{ $averageRating }} ({{ $mentor->user_ratings_count }} reviews)</span>
                 </div>
